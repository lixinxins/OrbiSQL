...
OrbiSQL 基于 Electron、React 与 TypeScript 构建，面向 macOS、Windows 和 Linux；仓库同时包含实验性的 HarmonyOS 客户端。项目当前支持 MySQL、PostgreSQL 和 SQLite，提供连接管理、对象浏览、SQL 查询、表结构设计及数据维护等常用能力。
...
- MySQL、PostgreSQL SSH 隧道连接，支持密码和私钥认证
- MySQL、PostgreSQL SSL/TLS 连接，支持 CA 及客户端证书
- 本地保存连接信息，密码通过 Electron `safeStorage` 加密
- 按数据库类型展示不同的对象树
- 数据库、数据表、字段、索引、外键、检查和触发器浏览
- 多标签 SQL 查询编辑器，支持语法高亮、检查、格式化和压缩
- 多语句批量执行、逐条结果与错误定位，支持 BEGIN / COMMIT / ROLLBACK 手动事务
- 保存和删除常用查询语句
