import { DownloadSimple, FileSql, FolderOpen, GearSix, GitDiff, Plus, Sparkle, UploadSimple } from '@phosphor-icons/react'
...
export default function HeaderToolbar({
  ...
  return (
    <header className="titlebar">
      {isMac && <div className="macrow" aria-hidden="true" />}
      ...
      <div className="brandrow">
        <div className="brand">
          ...
          <span className="brand-name">QuillDB</span>
        </div>
        ...
        <div className="actionbar">
          ...
          <button className="tool-button" onClick={onOpenAiDatabase} title="AI数据库">
            <Sparkle weight="fill" />
            <span>AI数据库</span>
          </button>
          ...
          <button className="tool-button" onClick={onOpenNewConnectionGroupDialog}>
            <FolderOpen weight="fill" />
            <span>新建分组</span>
          </button>
          <button className="tool-button" onClick={onExportConnections} title="导出数据库连接配置文件 (JSON)">
            <DownloadSimple />
            <span>导出连接</span>
          </button>
          <button className="tool-button" onClick={onImportConnections} title="从 JSON 配置文件导入数据库连接">
            <UploadSimple />
            <span>导入连接</span>
          </button>
          <button className="tool-button" onClick={() => onOpenAdvancedTool('schema')}>
            <GitDiff />
            <span>高级工具</span>
          </button>
          {isHarmonyPlatform() && (
            <button className="tool-button" onClick={onOpenSettingsDialog}>
              <GearSix />
              <span>设置</span>
            </button>
          )}
